...
# Low-Cost PM2.5 Sensors: Calibration Data for SDS011 and PMS7003

![Low-cost PM2.5 sensors: SDS011 and PMS7003](/assets/images/research/sensors/low_cost_sensors.jpg)
...
This study compares three sensors measuring fine particulate matter (PM2.5): the SDS011, PMS7003, and HPMA115S0. Sensor readings were compared with a reference station using a Federal Equivalent Method (FEM) monitor (MetOne BAM-1020). The goodness-of-fit (R2) ranged from 0.51 to 0.66, indicating moderate agreement between low-cost sensors and the reference. Despite limited accuracy, the fractional cost of these sensors enables dense air-quality monitoring networks that would be impractical with FEM instruments alone.
...
|-----------|---------|--------|-----------|-----------------|
| Price | ~$13 | ~$19 | ~$19 | ~$290 |
| Range (ug/m3) | 0-500 (1000 max) | 0-999 | 0-1000 | N/A |
| Stated error | 10%, +/-10 ug/m3 | 15%, +/-10 ug/m3 | 15%, +/-10 ug/m3 | N/A |
...
| Output | PM1, PM2.5, PM10 + 6 size bins | PM2.5, PM10 (float) | PM2.5, PM10 (int) | Small (>0.5um), Large (>2.5um) |
...
|-------|---------|--------|-----------|------------|
| US EPA | -- | -- | -- | R2 = 0.5-0.6 |
| AQ-SPEC | R2 = 0.85-0.97 (PMS5003 variants) | -- | -- | R2 = 0.81 |
| Johnston (2019), UK | rho = 0.88 | -- | rho = 0.85 | -- |
| Badura (2018), Poland | R2 = 0.73-0.75 | R2 = 0.66-0.70 | -- | -- |
| Liu (2018), Norway | -- | R2 = 0.71-0.80 | -- | -- |

Consensus from 10+ reports: SDS011 and PMS7003 overestimate PM2.5 relative to FEM devices, with field-test R2 values typically in the 0.6-0.8 range.
...
Raw data was filtered to remove single-peak anomalies exceeding 300 ug/m3. PMS7003 produced more anomalous peaks than SDS011. After cleaning, 1,451 valid hourly observations remained for analysis.

![60 days of continuous PM2.5 data](/assets/images/research/sensors/60days_data.png)
...
Both sensors overestimated PM2.5 relative to the BAM reference. The PMS7003 showed greater overestimation (slope 0.66 implies BAM reads ~66% of PMS7003 values) but better correlation. The strong R2 of 0.84 between the two low-cost sensors suggests the 5.3 km distance from the reference station accounts for some of the lower R2 values against BAM.
...
- At higher PM2.5, both sensors systematically overestimate relative to BAM
...
- SDS011 overestimated PM2.5 by ~30% relative to BAM
- PMS7003 overestimated PM2.5 by ~52% relative to BAM
- R2 values (0.51-0.66) are lower than other published studies, likely due to the 5.3 km separation and northeast wind patterns during winter/spring in Hanoi
- The strong inter-sensor correlation (R2 = 0.84) supports the hypothesis that distance, not sensor quality, is the primary source of disagreement with the reference
- Either sensor is suitable for low-cost PM2.5 monitoring with appropriate calibration against a local reference
