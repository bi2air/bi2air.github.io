...
TIME_COLS = ['sin_hour', 'cos_hour', 'sin_month', 'cos_month', 'is_day', 'is_weekend', 'is_spring', 'is_summer', 'is_fall', 'is_winter']
...
def prep_data_v3_temporal():
    print("Loading and preparing V3 + Temporal data...")
    ...
    df['is_day'] = ((df['hour'] >= 6) & (df['hour'] <= 18)).astype(float)
    df['is_weekend'] = (df['time'].dt.dayofweek >= 5).astype(float)
...
def train():
    X_train, y_train, X_valid, y_valid, c_train, c_valid, scaler_x = prep_data_v3_temporal()
    ...
    best_loss = float('inf')
    best_model_path = f'{MODELS_DIR}/t72_delta_skip_v3_temporal_best.pt'
    ...
    # Save V3 Temporal results
    with open('docs/results_v3_temporal.pkl', 'wb') as f:
