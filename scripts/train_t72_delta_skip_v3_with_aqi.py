...
def prep_data_v3_with_aqi():
    print("Loading and preparing V3 data...")
    ...
    # --- Generate AQI Probabilities ---
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print("Generating AQI probabilities using pre-trained classifier...")
    ...
    with torch.no_grad():
        ...
        aqi_train_logits = classifier(X_train_tensor)
        aqi_train_probs = torch.softmax(aqi_train_logits, dim=1) # shape: (batch, 5, 72)
        aqi_train_probs_flat = aqi_train_probs.view(X_train_tensor.shape[0], -1).cpu().numpy()
        
        X_valid_tensor = torch.FloatTensor(X_valid_scaled).to(device)
        aqi_valid_logits = classifier(X_valid_tensor)
        aqi_valid_probs = torch.softmax(aqi_valid_logits, dim=1)
        aqi_valid_probs_flat = aqi_valid_probs.view(X_valid_tensor.shape[0], -1).cpu().numpy()
        
    X_train_combined = np.hstack([X_train_scaled, aqi_train_probs_flat])
    X_valid_combined = np.hstack([X_valid_scaled, aqi_valid_probs_flat])
...
def train():
    X_train, y_train, X_valid, y_valid, c_train, c_valid = prep_data_v3_with_aqi()
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Training DeltaSkipMLP with AQI Features on {device} | Input Dim: {X_train.shape[1]}")
    ...
    best_loss = float('inf')
    best_model_path = f'{MODELS_DIR}/t72_delta_skip_v3_with_aqi_best.pt'
